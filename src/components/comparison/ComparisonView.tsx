...
import { useLatestComparison, useFiles } from '@/hooks/useFiles';
...
export function ComparisonView() {
  ...
  const GENERATION_COOLDOWN = 10000; // 10 seconds cooldown between attempts
  ...
  const handleRegenerateComparison = useCallback(async () => {
    ...
    try {
      ...
      // Set the data directly first to prevent the report from being cleared
      if (result && (result.summary || result.currentFile)) {
        queryClient.setQueryData(['comparisons', 'latest'], {
          ...result,
      ...
      // Then invalidate and refetch to ensure we have the latest
      ...
      if (refetchResult.data) {
        // Successfully got the report - keep hasAutoGenerated true to prevent re-triggering
        ...
        // Don't reset hasAutoGenerated here - let it stay true until report is actually loaded
      } else {
        // If refetch didn't work, the data should already be set from above
        console.log('[COMPARISON] Refetch failed, but data was set directly');
        if (!hasAutoGenerated.current) {
          toast({
            title: "Comparison Generated",
      ...
      // Only show error toast if manually triggered (before resetting flag)
      const wasAutoGenerated = hasAutoGenerated.current;
      // Reset on error so we can try again later
  ...
  // Automatically generate comparison if we have 2+ completed files but no comparison
  useEffect(() => {
    // Only auto-generate if we have a report (meaning generation was successful)
    // Reset the flag when we successfully get a report
    if (report && hasAutoGenerated.current) {
      console.log('[COMPARISON] Report loaded successfully, resetting auto-generation flag');
      hasAutoGenerated.current = false;
    ...
    const shouldAutoGenerate = 
      ...
      !isRegenerating &&
      timeSinceLastAttempt > GENERATION_COOLDOWN; // Cooldown to prevent rapid re-triggers

    if (shouldAutoGenerate) {
      console.log('[COMPARISON] Auto-generating comparison - found', completedFiles.length, 'completed files');
      hasAutoGenerated.current = true;
      lastGenerationAttempt.current = now;
      handleRegenerateComparison();
    }
  }, [report, isLoading, error, files, isRegenerating, handleRegenerateComparison]);

  // Refetch when component mounts or when regenerating completes
  useEffect(() => {
    // Refetch immediately when component mounts
    refetch();
    
    // Set up interval to refetch every 5 seconds if no data and not currently generating
    // This is less aggressive since auto-generation handles initial creation
    const interval = setInterval(() => {
      if (!report && !isRegenerating && !hasAutoGenerated.current) {
        refetch();
      }
    }, 5000);

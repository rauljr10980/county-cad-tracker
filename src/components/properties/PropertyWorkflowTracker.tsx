...
import type { Property, WorkflowStage, WorkflowLogEntry } from '@/types/property';
...
interface PropertyWorkflowTrackerProps {
  property: Property;
  onPropertyUpdate: (updated: Partial<Property>) => void;
}
...
export function PropertyWorkflowTracker({ property, onPropertyUpdate }: PropertyWorkflowTrackerProps) {
  ...
  const currentStage: WorkflowStage = (property.workflow_stage as WorkflowStage) || 'not_started';
  const workflowLog: WorkflowLogEntry[] = (property.workflow_log as WorkflowLogEntry[]) || [];
  ...
  const handleReset = async () => {
    ...
    setIsPending(true);
    try {
      await updatePropertyWorkflowStage(property.id, 'not_started', updatedLog);
      onPropertyUpdate({ workflow_stage: 'not_started', workflow_log: updatedLog });
      queryClient.invalidateQueries({ queryKey: ['properties'] });
      queryClient.invalidateQueries({ queryKey: ['tasks'] });
    ...
    <div className="bg-secondary/30 rounded-lg p-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        {currentStage !== 'not_started' && (
          <Button variant="ghost" size="sm" className="h-6 text-xs text-muted-foreground" onClick={handleReset} disabled={isPending}>
      ...
      <div className="border border-border rounded-lg p-3 space-y-3">
        {stageInfo.terminal ? (
          <div className="text-center py-2">
            ...
            <Button variant="outline" size="sm" onClick={handleReset} className="mt-3" disabled={isPending}>

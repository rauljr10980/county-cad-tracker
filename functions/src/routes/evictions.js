...
router.get('/landlords/:id', async (req, res) => {
  const item = await prisma.evictionLandlord.findUnique({ where: { id: req.params.id }, include: { _count: { select: { filings: true, addresses: true } }, addresses: { orderBy: { address: 'asc' } }, ownedProperties: { orderBy: { createdAt: 'asc' } }, filings: { orderBy: { filedDate: 'desc' }, take: 500 }, activities: { orderBy: { createdAt: 'desc' }, take: 100 }, tasks: { orderBy: { dueAt: 'asc' }, take: 100 }, assignedTo: { select: { id: true, username: true } } } });
